...
import { findAllApartments, saveApartment } from '@/app/components/apartment/service/apartment-service';
...
const RegisterPage: NextPage = () => {
    ...
    // }
    const handleCancel = () => {
        router.push(`pages/apartment/list`)
    }
    ...
    useEffect(() => {
        dispatch(findAllApartments())
    }, [])

    const apartmentList: [] = useSelector(getAllApartments)

    return (<>

        <form onSubmit={handleSubmit(onSubmit)} className="max-w-sm mx-auto">
            <label htmlFor="countries" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"> 게시판 선택 </label>
            <select {...register('boardId', { required: true })} id="countries" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 
